...
func load_export_config_from_file() -> Error:
	...
	if __load_result == Error.OK:
		is_real = bool(__config_file.get_value(CONFIG_FILE_SECTION_GENERAL, CONFIG_FILE_KEY_IS_REAL, false))
		debug_application_id = str(__config_file.get_value(CONFIG_FILE_SECTION_DEBUG, CONFIG_FILE_KEY_APP_ID, "ca-app-pub-3940256099942544~3347511713"))
		real_application_id = str(__config_file.get_value(CONFIG_FILE_SECTION_RELEASE, CONFIG_FILE_KEY_APP_ID, ""))
		...
		if __config_file.has_section(CONFIG_FILE_SECTION_MEDIATION):
			if __config_file.has_section_key(CONFIG_FILE_SECTION_MEDIATION, CONFIG_FILE_KEY_ENABLED_NETWORKS):
				var __raw_networks: Array = __config_file.get_value(CONFIG_FILE_SECTION_MEDIATION, CONFIG_FILE_KEY_ENABLED_NETWORKS, [])
				var __network_array: Array[String] = []
				for __net in __raw_networks:
					__network_array.append(str(__net))
